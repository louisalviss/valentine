...
isolation_capture = json.loads((ref_dir / 'evidence' / 'isolation' / 'capture.json').read_text())
...
if not scores:
    raise SystemExit(f'{reference_id}: no visual states to compare')
...
status = 'pass' if minimum >= 98.0 and interaction_ok else 'fail'
report = {
    'version': '1.0',
...
print(
    ...
    f"blocked={report['blocked_request_count']}"
)
